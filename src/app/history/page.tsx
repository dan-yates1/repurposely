...
export default function History() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (session?.user) {
        ...
        // We'll use the user state variable later
      } else {
        router.push("/auth");
      }
    };
  ...
  // Use the user variable when it changes
  useEffect(() => {
    if (user) {
  ...
  // Calculate analytics
  const twitterCount = contentHistory.filter(
    (item) => item.content_type === "twitter"
  ).length;
  const linkedinCount = contentHistory.filter(
    (item) => item.content_type === "linkedin"
  ).length;
  const blogCount = contentHistory.filter(
    (item) => item.content_type === "blog"
  ).length;
  const emailCount = contentHistory.filter(
    (item) => item.content_type === "email"
  ).length;
  const videoCount = contentHistory.filter(
          ...
          <div className="mb-6">
            <div className="relative">
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border text-gray-700 border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
              ...
              <div className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-100">
                {filteredHistory.slice(0, 5).map((item) => (
                  <HistoryCard
                    ...
                    description={
                      item.repurposed_content.substring(0, 60) +
                      (item.repurposed_content.length > 60 ? "..." : "")
                    }
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              <AnalyticsMetricCard
                ...
                title="Most Created"
                value={`${
                  twitterCount > linkedinCount && twitterCount > blogCount
                    ? "Twitter Threads"
                    : linkedinCount > blogCount
                    ? "LinkedIn Posts"
                    : "Blog Articles"
                } (${Math.max(twitterCount, linkedinCount, blogCount)})`}
              />
              <AnalyticsMetricCard
